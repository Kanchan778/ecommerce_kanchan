...
from .models import Product, Merch, Category, CartItem
...
def search(request):
    if request.method == "GET":
        ...
        if "-" in q:
            ...
            filter_query = Q(name__icontains=q) | Q(price__icontains=q) | Q(merch__name__icontains=q)
        products = Product.objects.filter(filter_query)
        categories = Category.objects.all()
        merches = Merch.objects.all()
        context = {
